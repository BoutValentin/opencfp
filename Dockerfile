...
ARG NODEVERSION=current

FROM node:${NODEVERSION}-alpine AS npm
...
WORKDIR /srv/app/
COPY package.json webpack.mix.js tailwind.js ./
...
COPY --from=npm /srv/app/resources resources/
COPY src src/
COPY tests tests/
COPY --from=npm /srv/app/web web/
COPY .php_cs.dist \
    app.json \
    migrations.php \
    mix-manifest.json \
